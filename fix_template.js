...
// Replace in !isIITBombay
const block1Start = \) : activeTab === 'Cutoffs' ? (\;
...
if (idx1Start !== -1) {
    ...
    if (idx1End !== -1) {
        const replacement1 = \) : activeTab === 'Round 1 Cutoffs' ? (
        ...
        console.log("Could not find block1End");
    }
} else {
    console.log("Could not find block1Start");
}

// Replace in isIITBombay
const block2Start = \) : activeTab === "Cutoffs" ? (\;
const block2End = \) : activeTab === "Placements" ? (\;
const idx2Start = content.indexOf(block2Start);
if (idx2Start !== -1) {
    const idx2End = content.indexOf(block2End, idx2Start);
    if (idx2End !== -1) {
        const replacement2 = \) : activeTab === 'Round 1 Cutoffs' ? (
            <div className="flex flex-col gap-8 min-w-0 animate-in fade-in duration-500">
    ...
    console.log("Could not find block2Start");
}
